...
import { deleteComment } from '../lib/supabaseClient'
...
export function CommentDeleteModal({
  ...
  const titleId = 'comment-delete-modal-title'
  const descId = 'comment-delete-modal-desc'
    ...
    >
      ...
      >
        ...
        <p id={descId} className="mb-4 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
          {commentPreview ? (
            <>
              <span className="line-clamp-2 block" style={{ color: 'var(--color-text-primary)' }}>
                “{commentPreview}”
              </span>
              <span className="mt-2 block">이 댓글을 삭제하려면 관리자 비밀번호를 입력하세요.</span>
            </>
          ) : (
            '이 댓글을 삭제하려면 관리자 비밀번호를 입력하세요.'
          )}
        </p>
